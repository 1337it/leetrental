(() => {
  ...
  class AdvancedLinkPicker {
    constructor({ doctype, targetField, frm, columns = [], staticFilters = {}, makeNew = true, pageLen = 20 }) {
      this.doctype = doctype;
      this.targetField = targetField; // fieldname in the form
      this.frm = frm;
    ...
    _buildDialog() {
      ...
      this.$qf = $head.find(".quick-filters");

      // Example quick filters (edit as needed)
      // You can add chips/dropdowns here:
      // e.g. status chip
      const statusSel = $(`<select class="input-sm form-control" style="width:160px">
          <option value="">Status: Any</option>
          <option value="Active">Status: Active</option>
          <option value="Inactive">Status: Inactive</option>
      </select>`);
      this.$qf.append(statusSel);

      // Results table
      const colHeads = this.columns.map(c => `<th style="width:${c.width || 'auto'}">${c.label}</th>`).join("");
      const $table = $(`
      ...
      const $foot = $(`
        <div class="flex items-center justify-between" style="margin-top:8px">
          ...
          <button class="btn btn-primary btn-sm create-new" ${this.makeNew ? "" : "style='display:none'"}>Create New ${this.doctype}</button>
        </div>
      `);
      ...
      statusSel.on("change", () => {
        this.state.quickFilters.status = statusSel.val() || undefined;
        this._freshSearch();
      });
  ...
  this.$search.trigger("focus");
    ...
    async _load() {
      ...
      if (this.state.quickFilters.status) filters.status = this.state.quickFilters.status;
      ...
      try {
        const { message } = await frappe.call({
          ...
          args: {
            ...
            start: pageStart,
            // Optional: custom query, searchfield, etc.
            filters,
          },
          freeze: false,
        });

        const rows = (message || []).map(r => ({
          value: r.value,
          description: r.description,
          // add more keys if your query returns them
          ...r
        }));

        this.state.rows.push(...rows);
        this.state.hasMore = rows.length === this.pageLen;
        this.state.page += 1;
    ...
    _renderRows() {
      ...
      if (!this.state.rows.length) {
        ...
        this.state.rows.forEach((row, idx) => {
          const tds = this.columns.map(c => `<td>${frappe.utils.escape_html(row[c.key] ?? "")}</td>`).join("");
    ...
    _onKey(e) {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        if (max >= 0) this._setActive(Math.min(this.state.selIndex + 1, max));
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        if (max >= 0) this._setActive(Math.max(this.state.selIndex - 1, 0));
      } else if (e.key === "Enter") {
        e.preventDefault();
        this._commitSelection();
      }
    ...
    _commitSelection() {
      ...
      if (this.frm && this.targetField) {
        this.frm.set_value(this.targetField, row.value);
      }
      this.dlg.hide();
    }
